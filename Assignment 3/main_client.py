...
if __name__ == "__main__":
    ...
    python_code_location = f"{protocol}/ClientApp.py"
    
    if protocol == "SelectiveRepeat":
        python_version = "python2.7"

    command = f"{python_version} {python_code_location} -a {sender_ip} -b {sender_port} -x {receiver_ip} -y {receiver_port} -m {sequence_number_bits} -w {window_size} -s {max_segment_size} -n {total_packets} -t {timeout} -z {sleep_interval} -d {directory_location} -f {filename}"
